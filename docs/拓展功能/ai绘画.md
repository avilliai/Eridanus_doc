...
```yaml
画 xxxxx
# prompt中随便一个地方加入横或方或竖就会变成画横图或方图或竖图，默认竖图
```
### tag反推
```yaml
tag
...
### 重绘
```yaml
重绘 prompt   # 比如 重绘 1girl,solo,loli，在prompt中随便一个地方加入横或方或竖就会变成画横图或方图或竖图，默认方图
n3re prompt  # nai3重绘，nai部分的重绘均为在prompt中随便一个地方加入横或方就会变成画横图和方图，默认竖图
n4re prompt  # nai4重绘，nai部分的重绘均为在prompt中随便一个地方加入横或方就会变成画横图和方图，默认竖图
```
然后发送图片
### 局部重绘
```yaml
局部重绘 prompt # sd局部重绘，在prompt中随便一个地方加入横或方或竖就会变成画横图或方图或竖图，默认方图
```
...
# 注:优先级是关键词中含有横或方或竖最高，其次你自定义的分辨率，nai相关部分不受自定义分辨率影响，同时横或宽不能超过1600
```
### 重绘中断指令
